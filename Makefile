...
install-dependencies: fmt xlnt rapidjson range-v3 std_date pybind11 websocketpp asio mongocxx
...
ifneq ($(PLATFORM),darwin)
	...
	$(warning mongocxx is NOT built on darwin)
endif
...
$(FMT_LOCAL_INCLUDE_PATHNAME): $(BUILD)
	...
	mkdir -p $(FMT_DIR)/build && \
	  cd $(FMT_DIR)/build && \
	  env CXX=$(CXX) cmake -D CMAKE_COLOR_MAKEFILE=OFF -DFMT_TEST=OFF -DFMT_DOC=OFF -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=TRUE -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_PREFIX="$(FMT_PREFIX)" -DCMAKE_PREFIX_PATH="$(FMT_PREFIX)" .. && \
	  $(MAKE) install
...
XLNT_CMAKE_CMD = env LDFLAGS= cmake -D CMAKE_COLOR_MAKEFILE=OFF -D CMAKE_BUILD_TYPE=Release -D TESTS=OFF -D CMAKE_CXX_FLAGS_RELEASE="$(XLNT_CXX_FLAGS)" -D CMAKE_CXX_COMPILER="$(CXX)" -DCMAKE_INSTALL_PREFIX="$(XLNT_PREFIX)" -DCMAKE_PREFIX_PATH="$(XLNT_PREFIX)" ..
...
openxlsx: $(AD_INCLUDE)
	...
	mkdir -p $(OPENXLSX_DIR)/build && \
	  ...
	  env LDFLAGS= cmake -D CMAKE_COLOR_MAKEFILE=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_PREFIX="$(OPENXLSX_PREFIX)" -DCMAKE_PREFIX_PATH="$(OPENXLSX_PREFIX)" .. && \
	  $(MAKE)
ifeq ($(PLATFORM),darwin)
